...
while (1):
    ...
    send_command = RTMP_DUMP_HEADER + ' -r "' + RTMP_HEADER + RTMP_IP + ":" \
    + RTMP_PORT + "/" + '" -y "' + RTMP_URL_HEADER + user_uuid + " - " \
    + IPCAM_ACCOUNT + " - " + IPCAM_PASSWORD + '" -Y -o ' + convert_locate + '.flv -v'
    print(send_command)
    record_OK = subprocess.Popen(send_command, shell = True)
    time.sleep(300)
    record_OK.kill()